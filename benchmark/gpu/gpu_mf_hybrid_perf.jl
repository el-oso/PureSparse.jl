...
for (nx,ny,nz) in [(28,28,28),(32,32,32),(40,40,40)]
    ...
    @printf("\ngrid3d %d³ n=%d nnzL=%d %.1f GF  CPU %.1f ms\n", nx,nx*ny*nz,S.nnzL,2*S.flops/1e9,cpu_t*1e3)
    best=(Inf,0.0,0)
    for qtl in [0.999,0.998,0.995,0.99,0.98,0.95,0.90]
        ...
        @printf("  q=%.3f GPU=%4d/%d  %.1f ms  %.2fx\n",qtl,ng,S.nsuper,t*1e3,cpu_t/t)
        t<best[1]&&(best=(t,qtl,ng))
    end
    @printf("  BEST %.1f ms at q=%.3f (%d GPU fronts) -> %.2fx\n",best[1]*1e3,best[2],best[3],cpu_t/best[1])
end

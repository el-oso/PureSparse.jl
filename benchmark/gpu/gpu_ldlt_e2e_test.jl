# M6b end-to-end: HYBRID multifrontal LDLᵀ factor + device LDLᵀ solve, vs ldlt! (design §6/§M).
...
function test(K, n1, n2, label)
    ...
    # symbolic + reference CPU LDLᵀ (same sym)
    ...
    dvec=Vector{Float64}(undef,n); d_dvec=CUDA.zeros(Float64,n)
    ...
    d_y = CuArray(b[perm]); d_upd=CUDA.zeros(Float64,mer); d_gath=CUDA.zeros(Float64,mer)
...
println("\nLDLᵀ END-TO-END PASSES — hybrid factor (L+D+inertia) + device solve match ldlt!")

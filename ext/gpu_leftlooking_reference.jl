# ==============================  REFERENCE (SUPERSEDED) — CUDA-ONLY  =============================
# The §4 left-looking supernodal Cholesky engines, retained ONLY as the measured baseline the
# multifrontal path (gpu_numeric.jl) beat — NOT on any shipped path (see the §M perf notes /
# crossover constants). These call cuSOLVER `potrf!` / cuBLAS `trsm!` directly, so they are CUDA-
# only and included solely by PureSparseCUDAExt. Shared helpers (_dpanel, _hpanel, gpu_gemm_nt!,
# gpu_assemble!, GPUSymbolic) come from gpu_shared.jl (included before this).
#
...
# trailing update, cuSOLVER potrf/trsm for the diagonal factor + panel solve).
#
...
# into x_host so it holds the complete factor (for the oracle + a CPU solve). CPU dense ops use
